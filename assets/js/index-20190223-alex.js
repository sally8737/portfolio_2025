$(function() {
  ...
  //  $this.addClass('active').siblings().removeClass('active')
    ...
    var gotoOffsetTop = $sallyDiv.eq($this.index()).offset().top,// - $('#nav_head').height()
      ...
      // time =  (bodySrcollTime < minSpeed ? minSpeed : bodySrcollTime > maxSpeed ? maxSpeed : bodySrcollTime) //計算卷軸動畫時間
      
      if(bodySrcollTime < minSpeed){
        bodySrcollTime = minSpeed
      }else if( bodySrcollTime > maxSpeed){
        bodySrcollTime = maxSpeed
      }
  ...
  function scrollFunction() {// mouse 滾動到第幾個單元, menu 那個單元的 li 裡面A 要加 
    if(animate) return
  ...
  $('.button-scrolly').click(function(){
    $('html,body').stop(true).animate({scrollTop:$('#portfolio').offset().top},500);
    }); 
